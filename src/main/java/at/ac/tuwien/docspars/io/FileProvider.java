...
public class FileProvider {
	...
	public void init() throws NullPointerException {
		List<File> filesList = Arrays.stream(new File(this.filePath).listFiles()).filter(f->f.getName().matches(VALID_FILE_PATTERN)).
		   // filter(f -> (f.exists() && f.isFile() && f.canRead() && !f.getName().matches(VALID_FILE_PATTERN))).
		    distinct().sorted((fa, fb) -> compareFile(fa.getName(),fb.getName())).collect(Collectors.toList());
