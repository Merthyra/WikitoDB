...
public class DictDAOdb
  ...
  public TObjectIntMap<ASCIIString2ByteArrayWrapper> read() {
    final ResultSetExtractor<TObjectIntMap<ASCIIString2ByteArrayWrapper>> resEx =
        new ResultSetExtractor<TObjectIntMap<ASCIIString2ByteArrayWrapper>>() {
          @Override
          public TObjectIntHashMap<ASCIIString2ByteArrayWrapper> extractData(final ResultSet res)
              throws SQLException, DataAccessException {
            final TObjectIntHashMap<ASCIIString2ByteArrayWrapper> dict =
                new TObjectIntHashMap<ASCIIString2ByteArrayWrapper>();
            while (res.next()) {
              dict.put(new ASCIIString2ByteArrayWrapper(res.getString("term")),
                  new Integer(res.getInt("tid")));
            }
            return dict;
          }
        };
