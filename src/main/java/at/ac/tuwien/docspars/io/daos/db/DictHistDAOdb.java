...
public class DictHistDAOdb implements DictHistDAO {
	...
	@Override
	public <B extends Dictionable> boolean update(final List<B> dict) {
		...
		final Map<Integer, Integer> dfUpdatedValues = this.jdbcTemplate.query(
				SQLStatements.getString("sql.dict_hist.readupdated") + " (" + idsB.toString() + ") GROUP BY tid",
				resEx);
		...
		final int[] updateCountsHist = this.jdbcTemplate.batchUpdate(SQLStatements.getString("sql.dict_hist.insert"),
	...
	@Override
	public <B extends Dictionable> boolean remove(final List<B> dicts) {
