...
public class DocumentTextProcessor {
  ...
  static {
    ...
    MAX_TERM_LENGTH = 95;
    try {
      ANALYZER = CustomAnalyzer.builder(Paths.get("src/main/resources")).withTokenizer("wikipedia")
          // .addTokenFilter("standard")
          .addTokenFilter("asciifolding").addTokenFilter("lowercase").addTokenFilter("elision")
          .addTokenFilter("stop", "ignoreCase", "true", "words", "stop-words_en.txt").addTokenFilter("porterstem").addTokenFilter("trim")
          .addTokenFilter("length", "min", "2", "max", String.valueOf(MAX_TERM_LENGTH)).build();
  ...
  public static List<String> tokenizeTextStream(String text) throws ParsingDocumentException {
    ...
    try (TokenStream tokens = ANALYZER.tokenStream("document", new StringReader(text))) {
      // tokens = ANALYZER.tokenStream("document", new StringReader(text));
      tokens.reset();

      while (tokens.incrementToken()) {
        // additionally remove all Tokens that would exceed configured column length
    ...
    logger.trace("Document content with {} characters split into {} terms of total characters {} : cut-ratio[%] {}", text.length(),
        textList.size(), finalNrOfCharacters, 100 - (finalNrOfCharacters * 100.0 / text.length()));
    return textList;
  }

  /**
   * normalize and deaccent string, removing all non US-ASCII characters (eg. 'a - > a)
   *
