...
public class SC2DocumentHandler extends DocumentHandler {
	...
	public void addPage(int pageID, int revID, String title, Timestamp timestamp, List<String> text) {
		...
		if (!this.getPersistedDocs().containsKey(pageID) || !this.getPersistedDocs().containsValue(pageID, new Document(pageID, revID, null, null, 0))) {
			// data structure stores all dict entries within a document to
			// access tf and df values
			...
			for (int i = 0; i < text.size(); i++) {
				...
				if (!this.getPersistedDict().containsKey(text.get(i))) {
					// first time a new dict element is beeing created, the df
					// value is set to one (1 document, the current document
					// contains it)
					// the tf value is also set to 1, as it corresponds to the
					// first occurrence within the document
					...
					// create new term and add it to the temporary list to be
					// stored
				...
				// the term is already stored in the sql dictionary
					...
					// now create new dict element to be lined up in dict
					// entries queue as copy of the persisted element but with
					// updated timestamp
					TimestampedDict newDictTerm = new TimestampedDict(tmpdic.getId(), text.get(i), timestamp, tmpdic.getDocFQ());

					// the term for the current document has already been found
					...
					// first occurrence of the term in the document
						...
						// increase the overall document frequency of the dict
						// term and add term to found documents
						// create new term
						Term t = new Term(tmpdic, newDoc, i + 1, 1);
						beenThere.put(tmpdic.getTerm(), t);
						// add it to termslist to be written to the db
						this.getNewTermEntries().add(t);
						// now relink dict entries with already stored dicts
						// (dict table contains dict elements, but the most
						// current one must be in front,
						// relink dict entries and update df values for all
						// others
	...
	/**
	 * helper mehtod which recursively iterates over single chained dict
	 * predecessor list and increments df values for all dicts more up to date
	 * than the new dict according to the timestamp provided
	 * 
	 * @param pointer
	 *            to the most current dict term instance or null if first dict
	 *            term doesnt change
	 */
